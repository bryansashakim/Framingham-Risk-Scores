...
get_frs = function(gender,age,HDL,TotChol,sbp,smoker,diabetic) {
  list_pts=c() # Initiate list of points
  #### FEMALES ####
  if (gender == 1) {
  ## GET POINTS FROM AGE ##
  i=sapply(age, function(p) { which(f_points$age_low <= p & f_points$age_high >= p)}); keep_df=f_points[-(setdiff(1:length(f_points$points),i)),];f_age <<- keep_df[[1]]
  ...
  if (smoker == 2) {list_pts=append(list_pts,0)} else {list_pts=append(list_pts,3)}
  f_smoke <<- 3
  ## GET POINTS FROM DIABETIC ##
  if (diabetic == 2) {list_pts=append(list_pts,0)} else {list_pts=append(list_pts,4)}
  f_diabetic <<- 4
  }
  #### MALES ####
 else {
    ## GET POINTS FROM AGE ##
  i=sapply(age, function(p) { which(m_points$age_low <= p & m_points$age_high >= p)}); keep_df=m_points[-(setdiff(1:length(m_points$points),i)),];m_age <<- keep_df[[1]]
  list_pts=append(list_pts,keep_df[[1]]) 
  ## GET POINTS FROM HDL ##
  ...
  list_pts=append(list_pts,keep_df[[1]])
  ## GET POINTS FROM SMOKER ##
  if (smoker == 2) {list_pts=append(list_pts,0)} else {list_pts=append(list_pts,4)}
  m_smoke <<- 4
  ## GET POINTS FROM DIABETIC ##
  if (diabetic == 2) {list_pts=append(list_pts,0)} else {list_pts=append(list_pts,3)}
  m_diabetic <<- 3
  }
  return(sum(list_pts))
}
